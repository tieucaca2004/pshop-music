...
async function handle(req, res, helpers) {
  ...
  const listPattern = /^\/api\/v1\/businesses\/([^/]+)\/payments$/;
  const itemPattern = /^\/api\/v1\/businesses\/([^/]+)\/payments\/([^/]+)$/;
  const callbackPattern = /^\/api\/v1\/businesses\/([^/]+)\/payments\/callback\/([a-z_]+)$/;
