/*
 * shared/aiModules.js — Sprint 14 Phase 5 (FINAL mục 17.19). Port NGUYÊN VĂN
 * `buildPrompt()`/`mapToDraftContent()` của 9 module `js/ai/modules/*.js` —
 * CÙNG prompt, CÙNG parse, CÙNG field. Client-side GIỮ NGUYÊN, không xoá —
 * vẫn là đường chạy thật cho Plugin Manager Dashboard/AI Assistant cho tới
 * khi UI được đổi sang gọi API mới (ngoài phạm vi Phase 5, "API only").
 *
 * Khác biệt bắt buộc so với client: `loadContext()` dùng
 * `shared/listResource.js` thay vì `DataProvider` (chỉ tồn tại phía client).
 * Vài helper trùng lặp giữa nhiều file client (`stripCodeFence`,
 * `parseJsonResponse`, `getYoutubeEmbedUrl`, `productCategoryLabels`) được
 * gộp lại 1 lần ở đây — hành vi giống hệt từng bản gốc, chỉ đỡ lặp code.
 */
...
const MODULES = {
  ...
  'blog-writer': {
    ...
    async loadContext(inputParams) { return productAndCategories(inputParams.productId); },
