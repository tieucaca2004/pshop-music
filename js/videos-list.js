(function () {
  ...
  function cardHtml(v, parsed) {
    const thumb = parsed.thumbnail
      ? `<img src="${escapeHtml(parsed.thumbnail)}" alt="${escapeHtml(v.title)}" loading="lazy">`
      : '';
    return `
      <div class="video-card">
        <div class="video-thumb" data-embed="${escapeHtml(parsed.embedUrl)}">
          ${thumb}
          <div class="video-play"><span>&#9658;</span></div>
        </div>
        <div class="video-card-title">${escapeHtml(v.title)}</div>
        ${v.description ? `<div class="video-card-desc">${escapeHtml(v.description)}</div>` : ''}
      </div>`;
  }
