...
const AdminAIObservability = (function () {
  ...
  function renderHealth(health) {
    if (health && health.error) return `<p style="color:#c0392b">Không kiểm tra được Health Check: ${PSH.escapeHtml(health.error)}</p>`;
    if (health && health.skipped) return `<p class="small-muted">${PSH.escapeHtml(health.message)}</p>`;
    ...
    return `
      ...
      <table class="admin-table">
        ...
        <tbody>
          <tr><td>AI Provider → Cloud Function → OpenAI API</td><td style="color:${health.provider.ok ? 'var(--gold-ink)' : '#c0392b'}">${health.provider.ok ? '✓ OK' : '✗ Lỗi'}</td><td>${PSH.escapeHtml(health.provider.message)}</td></tr>
          <tr><td>Queue (aiJobs)</td><td style="color:${health.queue.ok ? 'var(--gold-ink)' : '#c0392b'}">${health.queue.ok ? '✓ OK' : '✗ Lỗi'}</td><td>${PSH.escapeHtml(health.queue.message)}</td></tr>
          <tr><td>Draft Workflow (aiDrafts)</td><td style="color:${health.draftWorkflow.ok ? 'var(--gold-ink)' : '#c0392b'}">${health.draftWorkflow.ok ? '✓ OK' : '✗ Lỗi'}</td><td>${PSH.escapeHtml(health.draftWorkflow.message)}</td></tr>
        </tbody>
      </table>`;

  function renderProviders(providers) {
    if (providers && providers.error) return `<p style="color:#c0392b">Không đọc được danh sách Provider: ${PSH.escapeHtml(providers.error)}</p>`;
    ...
    return `<table class="admin-table">
      ...
      <tbody>${providers.map(p => `<tr>
        <td>${PSH.escapeHtml(p.label)}</td>
        <td style="color:${p.enabled ? 'var(--gold-ink)' : 'var(--ink-mute)'}">${p.enabled ? 'Đang bật' : 'Đang tắt'}</td>
        <td>${PSH.escapeHtml(p.model || '(chưa cấu hình)')}</td>
        <td>${p.active ? '✓' : ''}</td>
      </tr>`).join('')}</tbody></table>`;

  function renderQueue(queue) {
    if (queue && queue.error) return `<p style="color:#c0392b">Không đọc được Queue: ${PSH.escapeHtml(queue.error)}</p>`;
  ...
  function renderJobSummary(queue) {
    if (queue && queue.error) return `<p style="color:#c0392b">Không đọc được Job Summary: ${PSH.escapeHtml(queue.error)}</p>`;
    if (queue.total === 0) return '<p class="small-muted">Chưa có Job nào trong hệ thống.</p>';
    return `<table class="admin-table">
  ...
  function renderDrafts(drafts) {
    if (drafts && drafts.error) return `<p style="color:#c0392b">Không đọc được Draft Summary: ${PSH.escapeHtml(drafts.error)}</p>`;
  ...
  function renderPlugins(plugins) {
    if (plugins && plugins.error) return `<p style="color:#c0392b">Không đọc được trạng thái Plugin: ${PSH.escapeHtml(plugins.error)}</p>`;
    ...
    return `<table class="admin-table">
      ...
      <tbody>${plugins.map(p => `<tr>
        <td>${PSH.escapeHtml(p.name)}<br><span class="small-muted">${PSH.escapeHtml(p.id)}</span></td>
        <td style="color:${p.enabled ? 'var(--gold-ink)' : 'var(--ink-mute)'}">${p.enabled ? 'Đang bật' : 'Đang tắt (Coming Soon)'}</td>
        <td>${PSH.escapeHtml(p.provider || 'Mặc định toàn cục')}</td>
      </tr>`).join('')}</tbody></table>`;
  ...
  function renderUsage(usage, plugins, rangeKey) {
    if (usage && usage.error) return `<p style="color:#c0392b">Không đọc được Usage Summary: ${PSH.escapeHtml(usage.error)}</p>`;
    if (usage.total === 0) return `<p class="small-muted">Chưa có dữ liệu sử dụng AI trong khoảng "${PSH.escapeHtml(RANGE_LABELS[rangeKey])}".</p>`;
    return `
      <p><strong>Tổng số lần Generate (${PSH.escapeHtml(RANGE_LABELS[rangeKey])}): ${usage.total}</strong></p>
  ...
  function load() {
    ...
    ObservabilityService.compute(rangeKey).then(report => {
      ...
      root.innerHTML = `
        <p class="small-muted" style="margin-bottom:1rem">Cập nhật lần cuối: ${PSH.escapeHtml(new Date(report.generatedAt).toLocaleString('vi-VN'))}</p>
      ...
      `;
      root.innerHTML = '<p style="color:#c0392b">Lỗi khi tải AI Observability Dashboard: ' + PSH.escapeHtml(err.message) + '</p>';
