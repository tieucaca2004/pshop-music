...
const AdminAIHealth = (function () {
  ...
  function statusRow(label, result) {
    ...
    return `<tr>
      <td>${PSH.escapeHtml(label)}</td>
      <td style="color:${color}">${icon} ${result.ok ? 'OK' : 'Lỗi'}</td>
      <td>${PSH.escapeHtml(result.message)}</td>
    </tr>`;
    ...
    HealthCheck.run('openai').then(report => {
      resultEl.innerHTML = `
        <div class="panel">
          ...
          <p class="small-muted">Kiểm tra lúc ${PSH.escapeHtml(new Date(report.checkedAt).toLocaleString('vi-VN'))}</p>
      ...
      resultEl.innerHTML = '<p style="color:#c0392b">Lỗi khi chạy Health Check: ' + PSH.escapeHtml(err.message) + '</p>';
