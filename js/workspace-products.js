...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function loadCategories() {
    return CategoryDB.getAll().then(function (list) {
      ...
      select.innerHTML = '<option value="">-- Select --</option>' +
        categories.map(function (c) { return '<option value="' + escapeHtml(c.code) + '">' + escapeHtml(c.label) + '</option>'; }).join('');
    });
  }
  ...
  function render() {
    ...
    tbody.innerHTML = products.map(function (p) {
      return '<tr>' +
        '<td>' + (p.image ? '<img src="' + escapeHtml(p.image) + '" alt="">' : '') + '</td>' +
        '<td>' + escapeHtml(p.name) + '</td>' +
        '<td>' + escapeHtml(catLabel(p.category)) + '</td>' +
        '<td>' + escapeHtml(p.price) + '</td>' +
        '<td>' + escapeHtml(p.pubStatus || 'draft') + '</td>' +
        '<td>' +
