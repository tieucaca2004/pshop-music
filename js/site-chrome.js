...
const SiteChrome = (function () {
  ...
  function socialButtonHtml(s) {
    if (s.platform === 'zalo') {
      return `<a href="${escapeHtml(s.url)}" target="_blank" aria-label="Kết bạn Zalo" class="social-btn social-zalo">Z</a>`;
    }
    const icon = SOCIAL_ICONS[s.platform];
    if (!icon) return '';
    const cls = s.platform === 'facebook' ? 'fb' : s.platform;
    return `<a href="${escapeHtml(s.url)}" target="_blank" aria-label="${escapeHtml(s.platform)}" class="social-btn social-${cls}">${icon}</a>`;
  }
  ...
  function renderNav(menu, settings) {
    ...
    if (navLinks) {
      navLinks.innerHTML = items.map(i =>
        `<a href="${escapeHtml(i.link)}"${i.target === '_blank' ? ' target="_blank"' : ''}>${escapeHtml(i.label)}</a>`
      ).join('');
    }
    ...
    if (mobileNav) {
      let html = items.map(i =>
        `<a href="${escapeHtml(i.link)}"${i.target === '_blank' ? ' target="_blank"' : ''} onclick="this.closest('.mobile-nav').classList.remove('open')">${escapeHtml(i.label)}</a>`
      ).join('');
      if (settings) {
        if (settings.socials && settings.socials.facebook) {
          html += `<a href="${escapeHtml(settings.socials.facebook)}" target="_blank">Facebook</a>`;
        }
        if (settings.phone) {
          html += `<a href="tel:${escapeHtml(settings.phone)}" style="color:var(--gold);font-size:1.1rem">${escapeHtml(settings.phoneDisplay || settings.phone)}</a>`;
        }
      }
    ...
    if (grid && Array.isArray(footer.columns)) {
      grid.innerHTML = footer.columns.map((col, idx) => {
        ...
        const linksHtml = (col.links || []).map(l =>
          `<a href="${escapeHtml(l.link)}"${/^https?:/.test(l.link) ? ' target="_blank"' : ''}>${escapeHtml(l.label)}</a>`
        ).join('');
        if (isLast && Array.isArray(footer.social) && footer.social.length) {
          const socialHtml = footer.social.map(socialButtonHtml).join('');
          return `<div class="footer-col footer-col-news"><h5>${escapeHtml(col.title)}</h5><div class="footer-social">${socialHtml}</div>${linksHtml}</div>`;
        }
        return `<div class="footer-col"><h5>${escapeHtml(col.title)}</h5>${linksHtml}</div>`;
      }).join('');
    }
