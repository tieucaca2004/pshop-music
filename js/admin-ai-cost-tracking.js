...
const AdminAICostTracking = (function () {
  ...
  function costRows(costMap, labelFn) {
    ...
    return keys.map(k => `<tr><td>${PSH.escapeHtml(labelFn ? labelFn(k) : k)}</td><td>${costMap[k].count}</td><td>${formatCost(costMap[k])}</td></tr>`).join('');
  ...
  function renderReport() {
    ...
    CostTrackingService.compute(rangeKey).then(report => {
      if (report.total === 0) {
        ...
        resultEl.innerHTML = `<div class="panel"><p class="small-muted">Chưa có dữ liệu sử dụng AI trong khoảng "${PSH.escapeHtml(RANGE_LABELS[rangeKey])}" — ${UNAVAILABLE_TEXT}.</p></div>`;
        return;
      resultEl.innerHTML = `
        <div class="panel">
      ...
      resultEl.innerHTML = '<p style="color:#c0392b">Không đọc được dữ liệu Cost Tracking: ' + PSH.escapeHtml(err.message) + '</p>';
