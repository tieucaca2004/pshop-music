...
const AdminMediaLibrary = (function () {
  ...
  function load(searchTerm) {
    ...
    MediaLibrary.list(searchTerm).then(items => {
      if (!items.length) {
        grid.innerHTML = `<p class="small-muted">${searchTerm ? 'Không tìm thấy ảnh nào khớp "' + PSH.escapeHtml(searchTerm) + '".' : 'Chưa có ảnh nào trong Thư viện — tải ảnh lên để bắt đầu.'}</p>`;
        return;
      grid.innerHTML = items.map(item => `
        <div class="panel" style="padding:0.75rem" data-path="${PSH.escapeHtml(item.fullPath)}">
          <img src="${PSH.escapeHtml(item.url || '')}" alt="${PSH.escapeHtml(item.name)}" style="width:100%;height:120px;object-fit:cover;border-radius:6px;background:var(--bg-alt)">
          <p class="small-muted" style="margin:0.5rem 0 0;word-break:break-all">${PSH.escapeHtml(item.name)}</p>
          <button class="submit-btn ml-remove-btn" data-path="${PSH.escapeHtml(item.fullPath)}" style="width:100%;margin-top:0.5rem;background:var(--bg-alt);color:#c0392b">XOÁ</button>
        </div>`).join('');
