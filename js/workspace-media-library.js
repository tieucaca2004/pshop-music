...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function thumbHtml(item) {
    ...
    if (kind === 'image') {
      return '<img src="' + PSH.escapeHtml(item.url || '') + '" alt="' + PSH.escapeHtml(item.name) + '" style="width:100%;height:120px;object-fit:cover;border-radius:6px;background:var(--bg-alt)">';
    var icon = kind === 'video' ? '&#127909;' : '&#128196;';
  ...
  function load(searchTerm) {
    ...
    return MediaLibrary.list(searchTerm, { allTypes: true }).then(function (items) {
      if (!items.length) {
        WorkspaceAuth.renderEmpty(grid, searchTerm ? 'No files match "' + PSH.escapeHtml(searchTerm) + '".' : 'No media yet — upload a file to get started.');
        return;
      grid.innerHTML = items.map(function (item) {
        return '<div class="panel" style="padding:0.75rem" data-path="' + PSH.escapeHtml(item.fullPath) + '">' +
          thumbHtml(item) +
