...
## 9. Post-fix gap closure (2026-09-26)

| GAP | Phân loại | Tái hiện | Xử lý | Commit | Trạng thái |
|---|---|---|---|---|---|
| WF-E1 trần vòng lặp | DEFECT (an toàn) | `maxIterations: Infinity` treo (process con bị giết sau 5 s); 2.000.000 vòng khoá luồng 3,6 s; lồng nhau Infinity×Infinity treo | `MAX_LOOP_ITERATIONS = 100` ở khối Configuration của engine — **tái dùng** hạn mức AI có sẵn `aiGenerateDaily` = 100/ngày/uid (`functions/shared/rateLimit.js`, FINAL mục 7; dự án không có hằng số vòng lặp nào khác). Chỉ giới hạn giá trị VƯỢT trần (`capped:true`); ≤ trần và giá trị không hợp lệ giữ hành vi cũ | `aaa5bcc` | FROZEN (7 test) |
| WF-E2 event trước khi chờ | DESIGN CONTRACT (không phải lỗi) | `resumeExecution` trước `waitForEvent` → `{emitted:false, listeners:0}`, event không giữ | Hợp đồng A có chủ đích (registry chỉ giữ lượt chờ; nơi phát nhận `emitted:false`; không nơi nào phát event trước lượt chờ). Ghi rõ hợp đồng trong engine + 2 test khoá. Buffer (B) có rủi ro eventId toàn cục bị tiêu thụ nhầm → Founder quyết | `98aaa07` | FROZEN (hợp đồng A) |
| WF-C1 `config.timeout` trong `run()` | DEFECT (bất nhất trong engine) | step timeout 30 ms, executor 400 ms → chờ đủ 400 ms, báo completed; trong khi `run()` đã áp timeout cho `wait_event` và `runParallel` có timeout cùng semantics | Tái dùng `pTimeout()` của `runParallel` (thêm tham số `label` tuỳ chọn; thông báo của parallel giữ nguyên): hết giờ → step `failed` (thôi chờ, việc đang chạy không bị huỷ) → retry/dừng như mọi lỗi. Không áp `DEFAULT_TIMEOUT_MS` mặc định (tránh cắt ngang Workflow UI khi Queue tồn đọng) | `b98c598` | FROZEN (7 test) |
| WF-C2 FOREACH | DEFECT (trái hợp đồng FOREACH) | Mảng rỗng → rỗng ngay (đúng, A) nhưng thiếu `stoppedEarly`; mảng 3 item KHÔNG truyền builder → cả 3 bị đánh skipped, steps không chạy | Chỉ bỏ qua khi CÓ builder và builder trả null/`{skip:true}`; không builder → context cha (đúng quy ước `runLoop`); mảng rỗng cùng shape | `9a1f682` | FROZEN (4 test) |
| WF-C3 `status` job `workflow:auto` | DEFECT (state machine) | COMPLETED/FAILED/CANCELLED vẫn `status:'queued'` → poller/`GET /v1/jobs/:id` không thấy kết thúc | `status` phản chiếu `workflowState` (running/retrying/paused/cancelled/completed/failed); `UnlessStopped` ghi theo trạng thái thực sau transaction; nhánh dừng Cancel/Pause ghi qua `updateJobStatus()` có sẵn. Guard `status==='queued'` đọc snapshot LÚC TẠO (platform giao lại event dùng snapshot đó) → resume không đổi | `22c2b69` | FROZEN (6 test + 8 test cũ) |
| WF-S1 GenerationService | ARCHITECTURAL GAP / DEFERRED | Không HTML nào nạp `js/ai/services/*`; không loader động; server không dùng; `admin-ai.js`/`admin-image-ai.js` chỉ gọi `PipelineAdapter` sau `typeof !== 'undefined'` (luôn false) | Không refactor. Nếu được nạp: bỏ qua PermissionService (quyền theo plugin + log từ chối) và PluginManager (plugin tắt/thiếu field); DB Rules vẫn yêu cầu admin/editor. Thêm test khoá bất biến "không trang nào nạp" | `a8eaea1` | DEFERRED |

**Còn lại của WF-C3 (không sửa, LOW):** log FAILED của step `required:false` bị entry SKIPPED ghi đè; log SUCCESS không ghi draftId; enum `WAITING` không dùng.

**Cache-bust:** `393bbe3` bump `?v=9a1f682` (71 file HTML). Lệnh `sed` của lần bump này đổi nhầm 2 dòng lịch sử trong CHANGELOG/gap report → khôi phục ở `56893a9` (đối chiếu khớp `9b303b2`).

**Regression sau đợt này:** `workflow-engine` 68/68; `workflow-worker` 14/14 (Emulator); `npm test` 11/11; database-rules, storage-rules, registration-security, custom-claims-security: OK; E2E Workflow UI: Job + Log được tạo, lỗi dừng ở Provider, 0 lỗi JS; CMS: login/logout, sửa Banner/Video/Slider/Menu/Footer, Blog/Footer 5 trang public, redirect khi chưa đăng nhập: PASS; quét 64 trang y hệt baseline. Module FROZEN bị sửa: 0.
